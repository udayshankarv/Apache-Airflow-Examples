...
class MsSQLToS3Operator(BaseOperator):
    ...
    def __init__(self,
        ...
        self.s3_key = s3_key
        self.s3_key_suffix = s3_key_suffix
    ...
    def build_fetch_query(self, hook):
        # Builds the part of the fetch query with the incremental_key and condition
        ...
        if all([self.incremental_key, self.start, self.end]) and not self.condition:
            query_filter = """ WHERE {0} >= '{1}' AND {0} < '{2}'
                """.format(self.incremental_key, self.start, self.end)

        if all([self.incremental_key, self.start, self.end, self.condition]):
            query_filter = """ WHERE {0} >= '{1}' AND {0} < '{2}' AND {3}
                """.format(self.incremental_key, self.start, self.end, self.condition)

        if all([self.incremental_key, self.start]) and not self.end and not self.condition:
            query_filter = """ WHERE {0} >= '{1}'
                """.format(self.incremental_key, self.start)

        if all([self.incremental_key, self.start, self.condition]) and not self.end:
            query_filter = """ WHERE {0} >= '{1}' AND {2}
                """.format(self.incremental_key, self.start, self.condition)
    ...
    def get_records_batch(self, hook, query_filter):
       ...
       # if query_filter != 'WHERE':
        ...
        if s3_key_suffix:
            s3_key=s3_key.split(".")[0] + min_date+'-'+max_date +s3_key.split(".")[1]
